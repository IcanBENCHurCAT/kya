// Type definitions for Algorand wallet analysis
...
export interface WalletTransactionHistory {
  ...
  totalReceived: number; // in microAlgos
  totalSent: number; // in microAlgos
  netBalance: number; // in microAlgos
 ...
 */
export const DEFAULT_CONFIG: AlgorandConfig = {
  network: 'testnet',
  nodeURL: 'https://testnet-api.algonode.cloud',
  port: '',
  indexerURL: 'https://testnet-indexer.algonode.cloud',
  indexerPort: '',
  indexerToken: '',
};
...
export interface CacheStats {
  ...
  misses: number;
  size: number;
  entries: Record<string, number>; // key -> TTL remaining
}
