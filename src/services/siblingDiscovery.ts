...
export class SiblingDiscoveryService {
  ...
  discoverSiblings(
    ...
    const frequent = this.discoverFrequentCounterparties(
      address,
      transactions,
      counterpartyStats
    );
    ...
    const associated = this.discoverAssociatedWallets(
      address,
      transactions,
  ...
  private discoverFrequentCounterparties(
    ...
    if (counterpartyStats) {
      ...
      for (const tx of transactions) {
        const counterparty =
          tx.type === 'sent' ? tx.receiver : tx.sender;
      ...
      for (const [addr, stats] of counterpartyMap) {
        if (stats.count >= threshold) {
          siblings.push({
            ...
            confidence: this.calculateFrequentCounterpartyConfidence(
              stats.count,
              stats.value * -1, // No flow direction info from raw tx
              0,
              0
            ),
  ...
  private discoverAssociatedWallets(
    ...
    const senderToReceivers = new Map<
      string,
      { receivers: Set<string>; count: number; round: number }
    >();

    for (const tx of transactions) {
      if (tx.type === 'received') {
        const sender = tx.sender;
        if (!sender || excludeSet.has(sender)) continue;
    ...
    const commonSenders = new Map<string, number>();
