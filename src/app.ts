...
<footer><p>Protected by x402 micro-payments on Algorand. Built with Hono & TypeScript.</p></footer></main><script>function copyCmd(){var cmd=document.getElementById('curl-cmd')?document.getElementById('curl-cmd').innerText:'';var btn=document.getElementById('copy-btn');var status=document.getElementById('copy-status');var defaultLabel='Copy cURL health check command';function onSuccess(){if(btn){btn.innerText='Copied!';btn.classList.remove('failed');btn.classList.add('copied');btn.setAttribute('aria-label','Command copied to clipboard');btn.setAttribute('title','Command copied to clipboard');setTimeout(function(){btn.innerText='Copy';btn.classList.remove('copied');btn.setAttribute('aria-label',defaultLabel);btn.setAttribute('title',defaultLabel);},2000);}if(status){status.innerText='Command copied to clipboard';}}function onError(){if(btn){btn.innerText='Failed';btn.classList.remove('copied');btn.classList.add('failed');btn.setAttribute('aria-label','Failed to copy command to clipboard');btn.setAttribute('title','Failed to copy command to clipboard');setTimeout(function(){btn.innerText='Copy';btn.classList.remove('failed');btn.setAttribute('aria-label',defaultLabel);btn.setAttribute('title',defaultLabel);},2000);}if(status){status.innerText='Failed to copy command to clipboard';}}function fallbackCopy(text){try{var ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();var ok=document.execCommand('copy');document.body.removeChild(ta);return ok;}catch(e){return false;}}if(!navigator.clipboard||!navigator.clipboard.writeText){if(fallbackCopy(cmd)){onSuccess();}else{onError();}return;}navigator.clipboard.writeText(cmd).then(onSuccess).catch(function(){if(fallbackCopy(cmd)){onSuccess();}else{onError();}})}</script></body></html>`);
});
