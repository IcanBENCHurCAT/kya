...
.copy-btn{background:#334155;color:#f8fafc;border:none;padding:.35rem .75rem;border-radius:.375rem;cursor:pointer;font-size:.75rem;font-weight:500;white-space:nowrap;transition:all 0.15s ease}
...
<section aria-label="Quick Start API Example"><h2 style="font-size:1.1rem;color:#94a3b8;margin-top:1.5rem">Quick Start Command</h2><div class="code-box"><code id="curl-cmd" style="color:#e2e8f0;font-size:.875rem;word-break:break-all">curl -s /health</code><button id="copy-btn" type="button" onclick="copyCmd()" title="Copy cURL health check command" aria-label="Copy cURL health check command" class="copy-btn">Copy</button></div><div id="copy-status" class="sr-only" aria-live="polite"></div></section>
<footer><p>Protected by x402 micro-payments on Algorand. Built with Hono & TypeScript.</p></footer></main><script>function copyCmd(){var cmd=document.getElementById('curl-cmd')?document.getElementById('curl-cmd').innerText:'';var btn=document.getElementById('copy-btn');var status=document.getElementById('copy-status');var feedback=document.getElementById('copy-feedback');var defaultLabel='Copy cURL health check command';function onSuccess(){if(btn){btn.innerText='Copied!';btn.classList.remove('failed');btn.classList.add('copied');btn.setAttribute('aria-label','Command copied to clipboard');btn.setAttribute('title','Command copied to clipboard');}if(feedback){feedback.innerText='Copied!';feedback.className='copy-feedback success';}if(status){status.innerText='Command copied to clipboard';}setTimeout(function(){if(btn){btn.innerText='Copy';btn.classList.remove('copied');btn.setAttribute('aria-label',defaultLabel);btn.setAttribute('title',defaultLabel);}if(feedback){feedback.innerText='';feedback.className='copy-feedback';}},2000);}function onError(){if(btn){btn.innerText='Failed';btn.classList.remove('copied');btn.classList.add('failed');btn.setAttribute('aria-label','Failed to copy command to clipboard');btn.setAttribute('title','Failed to copy command to clipboard');}if(feedback){feedback.innerText='Failed';feedback.className='copy-feedback error';}if(status){status.innerText='Failed to copy command to clipboard';}setTimeout(function(){if(btn){btn.innerText='Copy';btn.classList.remove('failed');btn.setAttribute('aria-label',defaultLabel);btn.setAttribute('title',defaultLabel);}if(feedback){feedback.innerText='';feedback.className='copy-feedback';}},2000);}function fallbackCopy(text){try{var ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();var ok=document.execCommand('copy');document.body.removeChild(ta);return ok;}catch(e){return false;}}if(!navigator.clipboard||!navigator.clipboard.writeText){if(fallbackCopy(cmd)){onSuccess();}else{onError();}return;}navigator.clipboard.writeText(cmd).then(onSuccess).catch(function(){if(fallbackCopy(cmd)){onSuccess();}else{onError();}})}</script></body></html>`);
});
...
app.notFound((c) => {
  ...
  return c.html(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>404 — Page Not Found | KYA Service</title><style>
body{font-family:system-ui,-apple-system,sans-serif;background:#0f172a;color:#f8fafc;margin:0;padding:2rem 1rem;line-height:1.5}
.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border-width:0}
.sr-only:focus{position:static;width:auto;height:auto;padding:.5rem;background:#38bdf8;color:#0f172a;font-weight:600;border-radius:.25rem;display:inline-block;margin-bottom:1rem}
