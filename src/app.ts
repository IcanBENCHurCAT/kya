...
<footer><p>Protected by x402 micro-payments on Algorand. Built with Hono & TypeScript.</p></footer></main><script>function copyCmd(){var cmd=document.getElementById('curl-cmd')?document.getElementById('curl-cmd').innerText:'';var btn=document.getElementById('copy-btn');var status=document.getElementById('copy-status');var defaultLabel='Copy cURL health check command';if(!navigator.clipboard||!navigator.clipboard.writeText){if(btn){btn.innerText='Failed';btn.setAttribute('aria-label','Failed to copy command to clipboard');setTimeout(function(){btn.innerText='Copy';btn.setAttribute('aria-label',defaultLabel);},2000);}if(status){status.innerText='Failed to copy command to clipboard';}return;}navigator.clipboard.writeText(cmd).then(function(){if(btn){btn.innerText='Copied!';btn.classList.add('copied');btn.setAttribute('aria-label','Command copied to clipboard');setTimeout(function(){btn.innerText='Copy';btn.classList.remove('copied');btn.setAttribute('aria-label',defaultLabel);},2000);}if(status){status.innerText='Command copied to clipboard';}}).catch(function(){if(btn){btn.innerText='Failed';btn.setAttribute('aria-label','Failed to copy command to clipboard');setTimeout(function(){btn.innerText='Copy';btn.setAttribute('aria-label',defaultLabel);},2000);}if(status){status.innerText='Failed to copy command to clipboard';}});}</script></body></html>`);
});
