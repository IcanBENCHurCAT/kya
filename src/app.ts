...
.copy-btn.failed{background:#991b1b;color:#fca5a5}
.cmd-tab{background:#0f172a;color:#94a3b8;border:1px solid #334155;padding:.2rem .5rem;border-radius:.375rem;cursor:pointer;font-size:.75rem;font-weight:500;transition:all 0.15s ease}
...
<section aria-label="Quick Start API Example"><h2 style="font-size:1.1rem;color:#94a3b8;margin-top:1.5rem;margin-bottom:.5rem">Quick Start Command</h2><div role="tablist" aria-label="Quick start endpoint selector" style="display:flex;gap:.375rem;margin-bottom:.5rem"><button type="button" role="tab" id="tab-health" aria-selected="true" tabindex="0" aria-controls="curl-tabpanel" class="cmd-tab active" onclick="setCmd('/health',this)" onkeydown="handleTabKeydown(event,this)">/health</button><button type="button" role="tab" id="tab-x402" aria-selected="false" tabindex="-1" aria-controls="curl-tabpanel" class="cmd-tab" onclick="setCmd('/.well-known/x402.json',this)" onkeydown="handleTabKeydown(event,this)">x402.json</button><button type="button" role="tab" id="tab-agent" aria-selected="false" tabindex="-1" aria-controls="curl-tabpanel" class="cmd-tab" onclick="setCmd('/.well-known/agent-card.json',this)" onkeydown="handleTabKeydown(event,this)">agent-card.json</button></div><div id="curl-tabpanel" role="tabpanel" aria-labelledby="tab-health" aria-live="polite" aria-atomic="true" class="code-box"><code id="curl-cmd" style="color:#e2e8f0;font-size:.875rem;word-break:break-all">curl -s /health</code><button id="copy-btn" type="button" onclick="copyCmd()" title="Copy cURL health check command" aria-label="Copy cURL health check command" class="copy-btn"><svg id="copy-btn-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg><span id="copy-btn-text">Copy</span></button></div><div id="copy-status" class="sr-only" aria-live="polite"></div></section>
<footer><p>Protected by x402 micro-payments on Algorand. Built with Hono & TypeScript.</p></footer></main><script>var currentPath='/health';function getCopyLabel(){return currentPath==='/health'?'Copy cURL health check command':'Copy cURL '+currentPath+' command';}function setCmd(path,btn){currentPath=path;var tabs=document.querySelectorAll('.cmd-tab');for(var i=0;i<tabs.length;i++){tabs[i].classList.remove('active');tabs[i].setAttribute('aria-selected','false');tabs[i].setAttribute('tabindex','-1');}if(btn){btn.classList.add('active');btn.setAttribute('aria-selected','true');btn.setAttribute('tabindex','0');var panel=document.getElementById('curl-tabpanel');if(panel&&btn.id){panel.setAttribute('aria-labelledby',btn.id);}var status=document.getElementById('copy-status');if(status){status.innerText='Selected '+btn.innerText+' endpoint';}}updateCmdText();}function handleTabKeydown(e,btn){var tabs=Array.from(document.querySelectorAll('.cmd-tab'));var idx=tabs.indexOf(btn);if(idx===-1)return;var nextIdx=-1;if(e.key==='ArrowRight'||e.key==='ArrowDown'){nextIdx=(idx+1)%tabs.length;}else if(e.key==='ArrowLeft'||e.key==='ArrowUp'){nextIdx=(idx-1+tabs.length)%tabs.length;}else if(e.key==='Home'){nextIdx=0;}else if(e.key==='End'){nextIdx=tabs.length-1;}if(nextIdx!==-1){e.preventDefault();tabs[nextIdx].focus();tabs[nextIdx].click();}}function updateCmdText(){var origin=(window.location&&window.location.origin)?window.location.origin:'';var cmdEl=document.getElementById('curl-cmd');if(cmdEl){cmdEl.innerText='curl -s '+(origin?origin+currentPath:currentPath);}var btn=document.getElementById('copy-btn');if(btn&&!btn.classList.contains('copied')&&!btn.classList.contains('failed')){var lbl=getCopyLabel();btn.setAttribute('aria-label',lbl);btn.setAttribute('title',lbl);}}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',updateCmdText);}else{updateCmdText();}function copyCmd(){var origin=(window.location&&window.location.origin)?window.location.origin:'';var cmdEl=document.getElementById('curl-cmd');var cmd=cmdEl?cmdEl.innerText:('curl -s '+(origin?origin+currentPath:currentPath));var btn=document.getElementById('copy-btn');var btnText=document.getElementById('copy-btn-text')||btn;var status=document.getElementById('copy-status');var copySvg='<svg id="copy-btn-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>';var checkSvg='<svg id="copy-btn-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>';var errorSvg='<svg id="copy-btn-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>';function setIcon(svg){var iconEl=document.getElementById('copy-btn-icon');if(iconEl&&iconEl.parentNode){var temp=document.createElement('div');temp.innerHTML=svg;if(temp.firstChild)iconEl.parentNode.replaceChild(temp.firstChild,iconEl);}}function onSuccess(){if(btn){btn.classList.remove('failed');btn.classList.add('copied');btn.setAttribute('aria-label','Command copied to clipboard');btn.setAttribute('title','Command copied to clipboard');}if(btnText){btnText.innerText='Copied!';}setIcon(checkSvg);if(status){status.innerText='Command copied to clipboard';}setTimeout(function(){if(btn){btn.classList.remove('copied');btn.setAttribute('aria-label',getCopyLabel());btn.setAttribute('title',getCopyLabel());}if(btnText){btnText.innerText='Copy';}setIcon(copySvg);},2000);}function onError(){if(btn){btn.classList.remove('copied');btn.classList.add('failed');btn.setAttribute('aria-label','Failed to copy command to clipboard');btn.setAttribute('title','Failed to copy command to clipboard');}if(btnText){btnText.innerText='Failed';}setIcon(errorSvg);if(status){status.innerText='Failed to copy command to clipboard';}setTimeout(function(){if(btn){btn.classList.remove('failed');btn.setAttribute('aria-label',getCopyLabel());btn.setAttribute('title',getCopyLabel());}if(btnText){btnText.innerText='Copy';}setIcon(copySvg);},2000);}function fallbackCopy(text){try{var ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();var ok=document.execCommand('copy');document.body.removeChild(ta);return ok;}catch(e){return false;}}if(!navigator.clipboard||!navigator.clipboard.writeText){if(fallbackCopy(cmd)){onSuccess();}else{onError();}return;}navigator.clipboard.writeText(cmd).then(onSuccess).catch(function(){if(fallbackCopy(cmd)){onSuccess();}else{onError();}})}</script></body></html>`);
});
    ...
    return c.json(
      {
        ...
        discovery: { landing: "/", health: "/health" },
      },
      404,
    );
  }
  return c.html(
    `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>404 — Page Not Found | KYA Service</title><style>
body{font-family:system-ui,-apple-system,sans-serif;background:#0f172a;color:#f8fafc;margin:0;padding:2rem 1rem;line-height:1.5}
.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border-width:0}
.sr-only:focus{position:static;width:auto;height:auto;padding:.5rem;background:#38bdf8;color:#0f172a;font-weight:600;border-radius:.25rem;display:inline-block;margin-bottom:1rem}
...
const logoUrl =
  "https://raw.githubusercontent.com/IcanBENCHurCAT/kya/main/docs/kya_architecture_infographic.jpg";
...
const x402MetadataHandler = (c: any) => {
  ...
  return c.json({
    merchant: {
      name: "KYA Service — Trust Infrastructure for AI Agents",
      description:
        "On-chain Karma reputation ledgers, pre-flight A2A risk handshakes, zero-knowledge identity assertions, and multi-source sanctions screening gated by x402 micro-payments.",
      icon: logoUrl,
      image: logoUrl,
      iconUrl: logoUrl,
      icon_url: logoUrl,
    ...
    resources: [
      {
        ...
        description:
          "Query agent Karma score, tier, risk flags, and event history.",
        methods: ["GET"],
      ...
      },
      {
        ...
        description:
          "Execute machine-to-machine pre-flight risk evaluation and issue Ed25519-signed W3C Verifiable Credentials.",
        methods: ["POST"],
        networks: ["algorand:mainnet", "algorand:testnet"],
        tag: "x402-global-challenge",
      },
      {
        ...
        description:
          "Submit Groth16 Zero-Knowledge identity proof payloads (zero PII on-chain).",
        methods: ["POST"],
        networks: ["algorand:mainnet", "algorand:testnet"],
        tag: "x402-global-challenge",
      },
      {
        ...
        description:
          "Screen wallet address or beneficial owner identity against sanctions watchlists.",
        methods: ["POST"],
        networks: ["algorand:mainnet", "algorand:testnet"],
        tag: "x402-global-challenge",
      },
    ...
    skills: [
      {
        ...
        description:
          "Query dynamic agent reputation score, risk flags, and event history.",
      ...
      },
      {
        ...
        description:
          "Execute pre-flight risk evaluation and retrieve signed W3C Verifiable Credential trust passport.",
        tags: ["a2a", "trust", "passport", "x402-global-challenge"],
        examples: [
          "Run A2A handshake before dispatching bounty payment to target agent.",
        ],
      },
      {
        id: "kya_zk_proof",
        ...
        description:
          "Submit Groth16 Zero-Knowledge identity proof to upgrade verification tier without revealing PII.",
      ...
      },
      {
        ...
        description:
          "Screen wallet address or beneficial owner against OFAC SDN sanctions watchlists.",
...
app.use(
  ...
  x402PaymentGate({
    priceMicroAlgo: 1000,
    receiverAddress: configuredReceiver,
    tag: "x402-global-challenge",
  }),
);
...
async function main() {
  ...
  initializeWatchlist({}, false).catch((err) =>
    console.error("Watchlist init error:", err),
  );
  ...
  console.log(
    `   - GET  /api/v1/karma/:address — Query agent karma score & history`,
  );
  console.log(
    `   - POST /api/v1/karma/event  — Record karma credit/debit event`,
  );
  console.log(`   ── Human Verification ──`);
  console.log(
    `   - POST /api/v1/verify/email/initiate   — Start email OTP verification`,
  );
  console.log(
    `   - POST /api/v1/verify/email/complete   — Complete email OTP verification`,
  );
  console.log(
    `   - GET  /api/v1/verify/wallet/:address  — Check verification status`,
  );
  console.log(
    `   - GET  /api/v1/verify/identity/:hash   — Check by identity hash`,
  );
  console.log(
    `   - GET  /api/v1/verify/methods          — List available methods`,
  );
