 ...
 */
import { buildDefaultData, saveSanctionsList, loadSanctionsList, downloadOFACSDN, } from './ofac.js';
 ...
 */
export async function refreshWatchlists(lists, force = false) {
    if (state.running) {
 ...
 */
export function loadFallbackWatchlist() {
 ...
 */
export async function initializeWatchlist(lists, preferNetwork = false) {
    if (!preferNetwork) {
        // Try disk first
